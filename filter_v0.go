		...
		Query: graphql.NewObject(
			graphql.ObjectConfig{
				...
				Fields: graphql.Fields{
					...
					// Agregar el nuevo campo "personasPorEdad"
					"personasPorEdad": &graphql.Field{
						...
						Resolve: func(p graphql.ResolveParams) (interface{}, error) {
							edad, edadOK := p.Args["edad"].(int)

							if edadOK {
								// Buscar todas las personas con la edad especificada
								personasConEdad := make([]Persona, 0)
								for _, persona := range personas {
									if persona.Edad == edad {
										personasConEdad = append(personasConEdad, persona)
									}
								}
								return personasConEdad, nil
							}
